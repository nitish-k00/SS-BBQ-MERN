...
const OrderDetail = () => {
  ...
  const [satusChangeLoading, setSatusChangeLoading] = useState(false);
  const { id } = useParams();
        ...
        <tbody>
          {order.products.map((product) => (
            <tr key={product.product._id}>
              ...
              <td>
                {productId === product.product._id && satusChangeLoading ? (
                  <Spinner animation="border" role="status" />
                ) : (
                  <select
